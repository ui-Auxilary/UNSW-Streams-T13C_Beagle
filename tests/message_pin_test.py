...
def test_pin_no_messages_channel(clear_data, create_data):
    ...
    channel_pin = json.loads(channel_message_data.text)['messages'][0]['is_pinned']
    assert channel_pin == False
    channel_pin = json.loads(channel_message_data.text)['messages'][1]['is_pinned']
    assert channel_pin == False
    channel_pin = json.loads(channel_message_data.text)['messages'][2]['is_pinned']
    assert channel_pin == False
    channel_pin = json.loads(channel_message_data.text)['messages'][3]['is_pinned']
    assert channel_pin == False
    channel_pin = json.loads(channel_message_data.text)['messages'][4]['is_pinned']
    assert channel_pin == False
    channel_pin = json.loads(channel_message_data.text)['messages'][5]['is_pinned']
    assert channel_pin == False
...
def test_pin_no_messages_dm(clear_data, create_data):
    ...
    dm_pin = json.loads(dm_message_data.text)['messages'][0]['is_pinned']
    assert dm_pin == False
    dm_pin = json.loads(dm_message_data.text)['messages'][1]['is_pinned']
    assert dm_pin == False
    dm_pin = json.loads(dm_message_data.text)['messages'][2]['is_pinned']
    assert dm_pin == False
    dm_pin = json.loads(dm_message_data.text)['messages'][3]['is_pinned']
    assert dm_pin == False
    dm_pin = json.loads(dm_message_data.text)['messages'][4]['is_pinned']
    assert dm_pin == False
...
def test_double_pin_channel(clear_data, create_data):
...
def test_double_pin_dm(clear_data, create_data):
...
def test_user_is_not_owner_channel(clear_data, create_data):
    ...
    assert pin_data.status_code == 403

def test_user_is_not_owner_dm(clear_data, create_data):
    _, _, dm_messages, _, _, _, token_2 = create_data
    pin_data = requests.post(config.url + 'message/pin/v1', json={
        'token': token_2,
        'message_id': dm_messages[0]
    })
...
def test_invalid_token_and_pinned_message_channel(clear_data,create_data):
    token, channel_messages, _, _, _, _, _ = create_data
    requests.post(config.url + 'message/pin/v1', json={
        'token': token,
        'message_id': channel_messages[0]
...
def test_invalid_token_and_pinned_message_dm(clear_data,create_data):
    token, _, dm_messages, _, _, _, _ = create_data
    requests.post(config.url + 'message/pin/v1', json={
        'token': token,
        'message_id': dm_messages[0]
