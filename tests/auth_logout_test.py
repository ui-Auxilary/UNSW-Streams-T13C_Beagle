...
def create_data():
    ...
    token1 = json.loads(register_data.text)['token']
    ...
    token2 = json.loads(login_2_data.text)['token']

    # create a channel with that user
    requests.post(config.url + 'channels/create/v2', json={'token': token1,
                                                             'name': 'channel_1',
                                                             'is_public': True
                                                             })
...
def test_double_login_one_logout(clear_data, create_data):
    ...
    channels_list_data = json.loads(channels_list.text)

    assert channels_list_data == {'channels': [{'channel_id': 1, 'name': 'channel_1'}]}
