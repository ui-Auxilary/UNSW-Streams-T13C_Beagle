...
def test_cannot_login_old_email(clear_data, create_data):
    ...
    login_user_1 = requests.post(config.url + 'auth/login/v2', json={'email': 'asd@gmail.com',
                                                                     'password': 'qwertyuiop',
                                                                     })

    assert login_user_1.status_code == 400

    # verify that the user can login with the new email

    login_user_2 = requests.post(config.url + 'auth/login/v2', json={'email': 'newemail@gmail.com',
                                                                     'password': 'qwertyuiop',
                                                                     })

    assert login_user_2.status_code == 200
