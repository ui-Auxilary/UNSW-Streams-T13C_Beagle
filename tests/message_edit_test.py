...
def test_simple_case(clear_data, create_data):
    ...
    assert any(('message_id', message_id) in msg.items()
               for msg in channel_messages) == True
    assert any(('message', new_message) in msg.items()
               for msg in channel_messages) == False
...
def test_edit_dm_message(clear_data, create_dm_data):
    ...
    assert any(('message_id', message_id) in msg.items()
               for msg in dm_messages) == True
    assert any(('message', new_message) in msg.items()
               for msg in dm_messages) == False
...
def test_global_owner_no_permissions_if_not_dm_creator(clear_data, create_dm_data):
    token, _, dm_id, auth_user_id, token_2 = create_dm_data
    ...
    message_send_data_1 = requests.post(config.url + 'message/senddm/v1', json={
        'token': token_2,
        'dm_id': dm_id,
    ...
    message_id = json.loads(message_send_data.text)['message_id']
    message_id_2 = json.loads(message_send_data_1.text)['message_id']

    # check that user_2 can edit user_1's messages but not vice-versa
    # editing the target message_id to an empty string
    user_2_edit = requests.put(config.url + 'message/edit/v1', json={
        'token': token_2,
        'message_id': message_id,
        'message': "Changed message"
    })

    assert user_2_edit.status_code == 200
    ...
    assert user_1_edit.status_code == 403

    # editing the target message_id to an empty string
    user_1_remove_message = requests.delete(config.url + 'message/remove/v1', json={'token': token,
                                                                                    'message_id': message_id_2,
                                                                                    })

    assert user_1_remove_message.status_code == 403
