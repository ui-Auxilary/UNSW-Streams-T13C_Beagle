...
def test_single_dm(clear_data, create_data):
    ...
    assert dm_list[0] == {'name': 'christianlam, lawrencelee', 'dm_id': dm_id}

def test_no_dms(clear_data, create_data):
    token_1, _, _, _ = create_data
...
def test_multiple_dms(clear_data, create_data):
    ...
    get_list = requests.get(config.url + 'dm/list/v1', param = { 'token': token_2
                                                                })
    dm_list_2 = json.loads(get_list.text)['dms']

    get_list = requests.get(config.url + 'dm/list/v1', param = { 'token': token_3
                                                                })
...
def test_invalid_token(clear_data, create_data):
    ...
    assert  (dm_list.status_code == 400)
...
def test_group_dm(clear_data, create_data):    
    ...
    dm_list = json.loads(get_list.text)['dms']
    assert dm_list[0] == {'name': 'christianlam, lawrencelee',
                          'dm_id': dm_id}